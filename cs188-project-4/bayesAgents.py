...
def fillObsCPT(bayesNet, gameState):
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            newFactor = bn.Factor((OBS_VAR_TEMPLATE % obsPos), HOUSE_VARS, bayesNet.variableDomainsDict())
            top, bottom = False, False
            topLeft, bottomLeft = False, False
            if housePos == bottomLeftPos:
                bottom = True
                bottomLeft = True
                house = BOTTOM_LEFT_VAL
                impossible = TOP_LEFT_VAL
                same = BOTTOM_RIGHT_VAL
                diff = TOP_RIGHT_VAL
            elif housePos == topLeftPos:
                top = True
                topLeft = True
                house = TOP_LEFT_VAL
                impossible = BOTTOM_LEFT_VAL
                same = TOP_RIGHT_VAL
                diff = BOTTOM_RIGHT_VAL
            elif housePos == bottomRightPos:
                bottom = True
                topLeft = True
                house = BOTTOM_RIGHT_VAL
                impossible = TOP_RIGHT_VAL
                same = BOTTOM_LEFT_VAL
                diff = TOP_LEFT_VAL
            else:
                top = True
                bottomLeft = True
                house = TOP_RIGHT_VAL
                impossible = BOTTOM_RIGHT_VAL
                same = TOP_LEFT_VAL
                diff = BOTTOM_LEFT_VAL
            if bottom:
                newFactor.setProbability({FOOD_HOUSE_VAR:house, GHOST_HOUSE_VAR:same, (OBS_VAR_TEMPLATE % obsPos):RED_OBS_VAL}, PROB_FOOD_RED)
                newFactor.setProbability({GHOST_HOUSE_VAR:house, FOOD_HOUSE_VAR:same, (OBS_VAR_TEMPLATE % obsPos):RED_OBS_VAL}, PROB_GHOST_RED)
                newFactor.setProbability({FOOD_HOUSE_VAR:house, GHOST_HOUSE_VAR:same, (OBS_VAR_TEMPLATE % obsPos):BLUE_OBS_VAL}, 1-PROB_FOOD_RED)
                newFactor.setProbability({GHOST_HOUSE_VAR:house, FOOD_HOUSE_VAR:same, (OBS_VAR_TEMPLATE % obsPos):BLUE_OBS_VAL}, 1-PROB_GHOST_RED)
                newFactor.setProbability({FOOD_HOUSE_VAR:house, GHOST_HOUSE_VAR:same, (OBS_VAR_TEMPLATE % obsPos):NO_OBS_VAL}, 0)
                newFactor.setProbability({GHOST_HOUSE_VAR:house, FOOD_HOUSE_VAR:same, (OBS_VAR_TEMPLATE % obsPos):NO_OBS_VAL}, 0)
            else:
                newFactor.setProbability({FOOD_HOUSE_VAR:house, GHOST_HOUSE_VAR:same, (OBS_VAR_TEMPLATE % obsPos):RED_OBS_VAL}, PROB_FOOD_RED)
                newFactor.setProbability({GHOST_HOUSE_VAR:house, FOOD_HOUSE_VAR:same, (OBS_VAR_TEMPLATE % obsPos):RED_OBS_VAL}, PROB_GHOST_RED)
                newFactor.setProbability({FOOD_HOUSE_VAR:house, GHOST_HOUSE_VAR:same, (OBS_VAR_TEMPLATE % obsPos):BLUE_OBS_VAL}, 1-PROB_FOOD_RED)
                ...
                newFactor.setProbability({GHOST_HOUSE_VAR:house, FOOD_HOUSE_VAR:same, (OBS_VAR_TEMPLATE % obsPos):NO_OBS_VAL}, 0)
            if bottomLeft:
                newFactor.setProbability({FOOD_HOUSE_VAR:house, GHOST_HOUSE_VAR:diff, (OBS_VAR_TEMPLATE % obsPos):RED_OBS_VAL}, PROB_FOOD_RED)
                newFactor.setProbability({GHOST_HOUSE_VAR:house, FOOD_HOUSE_VAR:diff, (OBS_VAR_TEMPLATE % obsPos):RED_OBS_VAL}, PROB_GHOST_RED)
                newFactor.setProbability({FOOD_HOUSE_VAR:house, GHOST_HOUSE_VAR:diff, (OBS_VAR_TEMPLATE % obsPos):BLUE_OBS_VAL}, 1-PROB_FOOD_RED)
                newFactor.setProbability({GHOST_HOUSE_VAR:house, FOOD_HOUSE_VAR:diff, (OBS_VAR_TEMPLATE % obsPos):BLUE_OBS_VAL}, 1-PROB_GHOST_RED)
                newFactor.setProbability({FOOD_HOUSE_VAR:house, GHOST_HOUSE_VAR:diff, (OBS_VAR_TEMPLATE % obsPos):NO_OBS_VAL}, 0)
                newFactor.setProbability({GHOST_HOUSE_VAR:house, FOOD_HOUSE_VAR:diff, (OBS_VAR_TEMPLATE % obsPos):NO_OBS_VAL}, 0)
            else:
                newFactor.setProbability({FOOD_HOUSE_VAR:house, GHOST_HOUSE_VAR:diff, (OBS_VAR_TEMPLATE % obsPos):RED_OBS_VAL}, PROB_FOOD_RED)
                newFactor.setProbability({GHOST_HOUSE_VAR:house, FOOD_HOUSE_VAR:diff, (OBS_VAR_TEMPLATE % obsPos):RED_OBS_VAL}, PROB_GHOST_RED)
                newFactor.setProbability({FOOD_HOUSE_VAR:house, GHOST_HOUSE_VAR:diff, (OBS_VAR_TEMPLATE % obsPos):BLUE_OBS_VAL}, 1-PROB_FOOD_RED)
            ...
            newFactor.setProbability({FOOD_HOUSE_VAR:house, GHOST_HOUSE_VAR:impossible, (OBS_VAR_TEMPLATE % obsPos):RED_OBS_VAL}, PROB_FOOD_RED)
            newFactor.setProbability({FOOD_HOUSE_VAR:house, GHOST_HOUSE_VAR:impossible, (OBS_VAR_TEMPLATE % obsPos):BLUE_OBS_VAL}, 1-PROB_FOOD_RED)
            newFactor.setProbability({FOOD_HOUSE_VAR:house, GHOST_HOUSE_VAR:impossible, (OBS_VAR_TEMPLATE % obsPos):NO_OBS_VAL}, 0)

            newFactor.setProbability({FOOD_HOUSE_VAR:impossible, GHOST_HOUSE_VAR:house, (OBS_VAR_TEMPLATE % obsPos):RED_OBS_VAL}, PROB_GHOST_RED)
            newFactor.setProbability({FOOD_HOUSE_VAR:impossible, GHOST_HOUSE_VAR:house, (OBS_VAR_TEMPLATE % obsPos):BLUE_OBS_VAL}, 1-PROB_GHOST_RED)
            newFactor.setProbability({FOOD_HOUSE_VAR:impossible, GHOST_HOUSE_VAR:house, (OBS_VAR_TEMPLATE % obsPos):NO_OBS_VAL}, 0)

            newFactor.setProbability({FOOD_HOUSE_VAR:house, GHOST_HOUSE_VAR:house, (OBS_VAR_TEMPLATE % obsPos):RED_OBS_VAL}, PROB_FOOD_RED)
            newFactor.setProbability({FOOD_HOUSE_VAR:house, GHOST_HOUSE_VAR:house, (OBS_VAR_TEMPLATE % obsPos):BLUE_OBS_VAL}, 1-PROB_FOOD_RED)
            newFactor.setProbability({FOOD_HOUSE_VAR:house, GHOST_HOUSE_VAR:house, (OBS_VAR_TEMPLATE % obsPos):NO_OBS_VAL}, 0)

            newFactor.setProbability({FOOD_HOUSE_VAR:impossible, GHOST_HOUSE_VAR:impossible, (OBS_VAR_TEMPLATE % obsPos):NO_OBS_VAL}, 1)
            newFactor.setProbability({FOOD_HOUSE_VAR:same, GHOST_HOUSE_VAR:same, (OBS_VAR_TEMPLATE % obsPos):NO_OBS_VAL}, 1)
            newFactor.setProbability({FOOD_HOUSE_VAR:diff, GHOST_HOUSE_VAR:diff, (OBS_VAR_TEMPLATE % obsPos):NO_OBS_VAL}, 1)

            # next to neither ghost or food
            newFactor.setProbability({FOOD_HOUSE_VAR:same, GHOST_HOUSE_VAR:diff, (OBS_VAR_TEMPLATE % obsPos):NO_OBS_VAL}, 1)
            newFactor.setProbability({FOOD_HOUSE_VAR:diff, GHOST_HOUSE_VAR:same, (OBS_VAR_TEMPLATE % obsPos):NO_OBS_VAL}, 1)
            newFactor.setProbability({FOOD_HOUSE_VAR:same, GHOST_HOUSE_VAR:impossible, (OBS_VAR_TEMPLATE % obsPos):NO_OBS_VAL}, 1)
            newFactor.setProbability({FOOD_HOUSE_VAR:impossible, GHOST_HOUSE_VAR:same, (OBS_VAR_TEMPLATE % obsPos):NO_OBS_VAL}, 1)
            newFactor.setProbability({FOOD_HOUSE_VAR:diff, GHOST_HOUSE_VAR:impossible, (OBS_VAR_TEMPLATE % obsPos):NO_OBS_VAL}, 1)
            newFactor.setProbability({FOOD_HOUSE_VAR:impossible, GHOST_HOUSE_VAR:diff, (OBS_VAR_TEMPLATE % obsPos):NO_OBS_VAL}, 1)

            bayesNet.setCPT(OBS_VAR_TEMPLATE % obsPos, newFactor)
