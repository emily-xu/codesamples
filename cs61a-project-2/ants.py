...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if insect.is_queen and insect.is_imposter:
                self.bees.remove(insect)
                insect.place = None
            elif insect.container and insect.ant.is_queen and insect.ant.is_imposter:
                self.bees.remove(insect.ant)
                self.bees.remove(insect.ant.ant)
                insect.ant.place = None
                insect.ant.ant.place = None
            elif insect.is_queen or insect.container and insect.ant.is_queen:
                None
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        """
        # BEGIN Problem 3B
        count, location = 0, self.place
        while location != hive:
            if count >= self.min_range and count <= self.max_range and random_or_none(location.bees):
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            bee_list = [b.reduce_armor(self.damage) for b in self.place.bees[:: -1]]
        # END Problem 4A
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        ...
        p = self.place
        bee_list = p.bees
        for b in range(len(bee_list))[::-1]:
            bee_list[b].reduce_armor(self.damage) 
        # END Problem 6A


# BEGIN Problem 5B
class ScubaThrower(ThrowerAnt):
    """A ThrowerAnt that is more costly but doesn't lost armor in water."""
    name = 'Scuba'
    implemented = True
    food_cost = 5
    watersafe = True
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        if not self.is_queen:
            ...
            a_place = self.place.exit
            while a_place:
                if a_place.ant:
                    if a_place.ant not in QueenAnt.doubled:
                        a_place.ant.damage *= 2
                        QueenAnt.doubled.append(a_place.ant)
                    if a_place.ant.container and a_place.ant.ant and a_place.ant.ant not in QueenAnt.doubled:
                            a_place.ant.ant.damage *= 2
                            QueenAnt.doubled.append(a_place.ant.ant)
                a_place = a_place.exit
        # END Problem 9
