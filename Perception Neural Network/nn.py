...
class Graph(object):
    ...
    def add(self, node):
        ...
        inputs = [self.outputs[parent] for parent in node.get_parents()]
        self.inputs[node] = inputs
        outputs = node.forward(inputs)
        self.outputs[node] = outputs
        self.gradients[node] = np.zeros_like(outputs)
    ...
    def backprop(self):
        ...
        nodes_backward = self.get_nodes()[::-1]
        for node in nodes_backward:
            gradient = node.backward(self.get_inputs(node), self.get_gradient(node))
            for i in range(len(node.get_parents())):
                parent = node.get_parents()[i]
                self.gradients[parent] = self.gradients[parent] + gradient[i]
    ...
    def step(self, step_size):
        ...
        for v in self.nodes:
            if isinstance(v, Variable):
                v.data = v.data - np.multiply(self.get_gradient(v), step_size)
...
class MatrixVectorAdd(FunctionNode):
    ...
    def forward(inputs):
        matrix = inputs[0]
        vector = inputs[1]
        new = np.broadcast_to(vector, matrix.shape)
        return matrix + new
...
class ReLU(FunctionNode):
    ...
    @staticmethod
    def forward(inputs):
        z = np.zeros_like(inputs[0])
        return np.where(inputs[0] < 0, z, inputs[0])

    @staticmethod
    def backward(inputs, gradient):
        z = np.zeros_like(gradient)
        return [np.where(inputs[0] < 0, z, gradient)]
...
class SquareLoss(FunctionNode):
    ...
    @staticmethod
    def backward(inputs, gradient):
        diffA = inputs[0] - inputs[1]
        diffB = inputs[1] - inputs[0]
        prodA = np.dot(diffA, gradient/len(inputs[0]))
        prodB = np.dot(diffB, gradient/len(inputs[0]))
        return [prodA, prodB]
