...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhosts = gameState.getLivingGhosts() # list of true/false
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]] # dictionary (position): probability
        ...
        for posDist in livingGhostPositionDistributions:
            ...
            for pos in posDist.keys():
                if posDist[pos] > maxProb:
                    maxProb = posDist[pos]
                    maxPos = pos
            ghostPos[maxPos] = self.distancer.getDistance(pacmanPosition, maxPos)
        closestGhost = min(ghostPos)
        action = None
        distToClosestGhost = self.distancer.getDistance(pacmanPosition, closestGhost)
        for a in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, a)
            newdist = self.distancer.getDistance(successorPosition, closestGhost)
            if newdist < distToClosestGhost:
                distToClosestGhost = newdist
                action = a
        return action
